...
router.post("/", (req, res) => {
  ...
  db.query(
    `INSERT INTO Book (title, genre, publication_year, author_id, bookshop_id) VALUES (?, ?, ?, ?, ?)`,
...
router.put("/:id", (req, res) => {
  const { title, genre, publication_year, author_id, bookshop_id } = req.body;
  db.query(
    `UPDATE Book SET title = ?, genre = ?, publication_year = ?, author_id = ?, bookshop_id = ? WHERE book_id = ?`,
